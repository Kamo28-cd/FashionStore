...
$.ajax({
	...
	success: function(r) {							
		...
		$.each(r, function(index) {
			if (r[index].in_stock == 'yes') {
				$('.ul-color').html(
				$('.ul-color').html() + '<input data-colorid="'+r[index].color_id+'" class="selectColor" style="display:none" type="radio" id="'+r[index].color_name+'" name="color" value="'+r[index].color_name+'"><label style="background: linear-gradient(45deg, '+r[index].color_code_1+', '+r[index].color_code_1+' 100%),linear-gradient(135deg, '+r[index].color_code_2+', '+r[index].color_code_2+'),linear-gradient(225deg, '+r[index].color_code_1+', '+r[index].color_code_1+'),linear-gradient(225deg, '+r[index].color_code_2+', '+r[index].color_code_2+');background-size: 50% 50%;background-position: 0% 0%, 0% 100%, 100% 0%, 100% 100%;background-repeat: no-repeat;" class="'+r[index].color_name+'" for="'+r[index].color_name+'"></label>')
