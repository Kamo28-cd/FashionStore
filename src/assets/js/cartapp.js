...
class Products{
	async getProducts() {
		try {
			...
			let result = await fetch('.assets/products.php?response=all');
	...
	setCartValues(cart) {
		...
		cart.map(item =>{
			...
			if (cart.find(myitem=> myitem.id ===1)) {
            			...
            			console.log('neither 1 nor 5 are currently inside')
                        if ((itemid == '1') || (itemid == '5')) {
                        	console.log('currently adding 1 or currently adding 5')
                            if (itemsTotal >=1) {
                            	myshipping =item.shipping* 1 -100 ;
                                 console.log('shipping is 0 but subtracting 100 because something other than 1 or 5 is inside')
                            } else if (itemsTotal <1) {
                            	myshipping =item.shipping* 1 ; 
                                console.log('shipping is 0 nothing else is inside')
                            }
                        } else if ((itemid != '1') && (itemid != '5') && (itemid != 0)) {
                        	console.log('currently not adding 1 or currently not adding 5')
                            if (itemsTotal <1) {
                            	myshipping =item.shipping* 1 ;
                        	...
                        	if (!cart.find(myitem=> myitem.id ===1)) {
                        		myshipping =item.shipping* 1 ;
                                console.log('not adding, 1 is not inside shipping should be 100 (value of the item that is inside)')
                            } else if (!cart.find(myitem=> myitem.id ===5)) {
                            	myshipping =item.shipping* 1 ;
                                console.log('not adding, 5 is not inside shipping should be 100 (value of the item that is inside)')
                            }
                        }
            }
			//console.log(myshipping)
